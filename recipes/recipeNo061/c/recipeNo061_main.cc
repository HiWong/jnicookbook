...
int main() {
  ...
  // First of all, we have to create HashMap, instantiate it and fill it with data
  // coming from 'map' object.
  ...
  jclass cls_Main = env->FindClass ("recipeNo061/Main");
  if(cls_Main == NULL) {
    return 6;
  }
  
  // This is the place where we call the method "displayMessage"
  jmethodID method_displayMap = env->GetStaticMethodID (cls_Main, "displayMap", "(Ljava/util/HashMap;)V");
  ...
  jobject result = env->CallStaticObjectMethod(cls_Main, method_displayMap, hashMap);
  
  // Note that you can check 'result' for the previous value associated with 'key'.
  // The same way, you can do in Java:
