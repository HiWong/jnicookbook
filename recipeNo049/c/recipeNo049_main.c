...
void redirect_std_out (JNIEnv *env) {
    ...
    // System.out
    jclass fos_class        = (*env)->FindClass(env, "java/io/FileOutputStream");
    jmethodID fos_constructor = (*env)->GetMethodID(env, fos_class, "<init>", "(Ljava/lang/String;)V");
    jobject fos_object      = (*env)->NewObject(env, fos_class, fos_constructor, (*env)->NewStringUTF(env, "jvm.log"));

    // Create a PrintStream
    jclass ps_class         = (*env)->FindClass(env, "java/io/PrintStream");
    jmethodID ps_constructor  = (*env)->GetMethodID(env, ps_class, "<init>", "(Ljava/io/OutputStream;)V");
    jobject ps_object       = (*env)->NewObject(env, ps_class, ps_constructor, fos_object);

    // Reassign System.out
    jclass system_class     = (*env)->FindClass(env, "java/lang/System");
    jmethodID method_id      = (*env)->GetStaticMethodID(env, system_class, "setOut", "(Ljava/io/PrintStream;)V");
...
void invoke_class (JNIEnv * env) {
  ...
  // and it's method.
  main_class = (*env)->FindClass (env, "recipeNo049/Main");
